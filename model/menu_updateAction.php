...
class menu
{
    ...
    public function menu_inster($restName,$num, $kind, $price, $action, $menu_tmpname, $menu_name)
    {
        ...
        {
            mysqli_query($db, "UPDATE menu SET `kind` = '$kind' WHERE `num`='$num'");  //修改菜單名稱
            mysqli_query($db, "UPDATE menu SET `price` = '$price' WHERE `num`='$num'");  //修改菜單單價

            if (!move_uploaded_file($menu_tmpname, "photo/" . $menu_name)) {        //執行上傳
                header("Location:menu_select.php?restname=$restName");
            } else {
